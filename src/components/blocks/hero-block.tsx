...
export function HeroBlock({ data, project }: HeroBlockProps) {
  ...
  if (variant === "banner") {
    return (
      <section className="mb-12 border-4 border-black bg-white">
        {heroImage && (
          <div className="relative w-full border-b-4 border-black overflow-hidden" style={{ aspectRatio: "16/9" }}>
            <img src={heroImage} alt={project?.title || "Hero"} className="absolute inset-0 w-full h-full object-cover" />
          </div>
        )}
  ...
  if (variant === "banner-compact") {
    return (
      <section className="mb-12 border-4 border-black bg-white">
        {heroImage && (
          <div className="relative w-full border-b-4 border-black overflow-hidden" style={{ aspectRatio: "21/9" }}>
            <img src={heroImage} alt={project?.title || "Hero"} className="absolute inset-0 w-full h-full object-cover" />
          </div>
        )}
    ...
    return (
      <section className="mb-12 border-4 border-black bg-white">
        <div className="grid grid-cols-1 md:grid-cols-2">
          ...
          {heroImage && (
            <div className="relative min-h-[300px] md:min-h-0 bg-gray-100 overflow-hidden">
              <img src={heroImage} alt={project?.title || "Hero"} className="absolute inset-0 w-full h-full object-cover" />
            </div>
          )}
        </div>
