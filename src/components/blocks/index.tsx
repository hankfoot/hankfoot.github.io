// Block registry used by the /dev/blocks style guide.
// Production project pages import block components directly in their MDX files.
...
function SectionHeader({ number, label, color }: { number: number; label: string; color: string }) {
  return (
    <div className="flex items-center gap-3 text-white p-3 border-b-4 border-black" style={{ backgroundColor: color }}>
      ...
      {groups.map((group, gi) => {
        ...
        return (
          <div key={`group-${gi}`} className="border-4 border-black bg-white mb-12 overflow-hidden">
            <SectionHeader number={group.section.number} label={group.section.label} color={group.section.color} />
