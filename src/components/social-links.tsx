...
export function SocialLinks({ social, variant }: SocialLinksProps) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {social.bluesky && (
        <a href={social.bluesky} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center border-4 border-black text-black hover:bg-black hover:text-white font-black px-8 py-3 text-sm transition-colors">
          <BlueskyIcon className="mr-2 h-4 w-4" /> BLUESKY
        </a>
      )}
      {social.linkedin && (
        <a href={social.linkedin} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center border-4 border-black text-black hover:bg-black hover:text-white font-black px-8 py-3 text-sm transition-colors">
          <Linkedin className="mr-2 h-4 w-4" /> LINKEDIN
        </a>
      )}
      {social.resume && (
        <a href={social.resume} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center bg-black text-white hover:bg-gray-800 font-black px-8 py-3 border-2 border-black text-sm transition-colors">
